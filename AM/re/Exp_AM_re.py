...
for i in range (len(s1_datasets)):
    ...
    for seeds in range(5):  
        ...
        s1_x_train_GB = torch.FloatTensor(s1_data[0:ini_train_size, :-1])
        s1_y_train_GB = torch.LongTensor(s1_data[0:ini_train_size, -1])
        ...
        s2_x_train_GB = torch.FloatTensor(s2_data[0:ini_train_size, :-1])
        s2_y_train_GB = torch.LongTensor(s2_data[0:ini_train_size, -1])
        ...
        s3_x_train_GB = torch.FloatTensor(s3_data[0:ini_train_size, :-1])
        s3_y_train_GB = torch.LongTensor(s3_data[0:ini_train_size, -1])
        ...
        if len(np.unique(s1_y_train)) == 1:
            ...
            s1_resampled_100 = resample(s1_resampled_data, n_samples=100, random_state=42)
            ...
            s2_resampled_100 = resample(s2_resampled_data, n_samples=100, random_state=42)
            ...
            s3_resampled_100 = resample(s3_resampled_data, n_samples=100, random_state=42)
        ...
        s1_f1 = f1_score(s1_y, s1_y_pred_cum, labels = labels, average = 'macro')
        s2_f1 = f1_score(s2_y, s2_y_pred_cum, labels = labels, average = 'macro')
        s3_f1 = f1_score(s3_y, s3_y_pred_cum, labels = labels, average = 'macro')
        print('S1 f1:', s1_f1)
        print('S2 f1:', s2_f1)
        print('S3 f1:', s3_f1)
